...
describe("renderX12", () => {
  ...
  it("uses action 4 for a verify file and omits a null benefit plan", () => {
    ...
    expect(text).toContain("REF*0F*CARD123~");
    expect(text).toContain("REF*17*01~");
    expect(text).not.toContain("REF*1L*PLAN-1~");
    expect(text).toContain("NM1*IL*1*MADONNA*****34*CARD123~");
    expect(text).not.toContain("DMG*D8*");
    expect(text).toContain("HD*001**PDG*UNKNOWN*EMP~");
    expect(text).not.toContain("DTP*356*D8*");
    expect(text).not.toContain("DTP*357*D8*");
    expect(text).not.toContain("DTP*348*D8*");
    expect(text).not.toContain("DTP*349*D8*");
  });
});
