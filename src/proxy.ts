...
function nextForSponsor(request: NextRequest) {
  if (demoFeaturesEnabled() && request.cookies.get("glass_demo_sponsor")?.value === "tennessee") {
    ...
    const allowed=["/contract-extraction","/api/contract-extraction","/contracts/wisconsin-etg0013-amendment-1.pdf","/sponsor","/sponsor/assurance","/rebate-protection","/contract-checks","/api/contract-checks","/api/operational-assurance","/api/operational-assurance/process","/api/demo-sponsor","/leakage-challenges","/api/leakage-challenges","/agents/leakage-challenge","/agents/rebate-protection","/data-agent-checks","/api/data-agent-checks","/agents/data-agent-tester"];
