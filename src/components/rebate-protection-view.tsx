...
export function RebateProof({run}:{run:Simulation}){
 ...
 </section><section className={styles.panel}><h2>Assumptions & variance</h2><p>Root cause: {s.scenario.cause}. Utilization is fixed in both paths.</p><ul className={styles.list}>{s.assumptions.map(a=><li key={a}>{a}</li>)}</ul><p className={styles.note}>A fixed expected-results test fixture independently checks the engine. These scenarios demonstrate control behavior, not actual savings or an independently audited result.</p><details><summary>Frozen contract versions & claims</summary><pre>{JSON.stringify(s,null,2)}</pre></details></section><RebateExceptions run={run}/><RebateHistory run={run}/></>;
}
