...
interface ReplayWorld {
  ...
  drugs: Map<string, EngineDrug & { isBrandLabel: boolean }>;
...
export async function loadWorld(force = false): Promise<ReplayWorld> {
  ...
  const drugs = new Map<string, EngineDrug & { isBrandLabel: boolean }>();
...
export async function replay(
  ...
  const claims = await prisma.$queryRaw<ReplayClaimRow[]>`
    SELECT c.id, c.claimNumber, c.memberId, c.drugId, c.pharmacyId,
           ...
           c.ingredientCostSubmittedCents, c.responseStatus, c.rejectCodes,
