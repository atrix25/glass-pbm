...
MUTED_ON_DARK = RGBColor(0xA8, 0xB0, 0xBE)
BODY_ON_DARK = RGBColor(0xC5, 0xCB, 0xD4)
...
def eyebrow(slide, text, top=Inches(0.45), dark=False):
    box = slide.shapes.add_textbox(Inches(0.7), top, Inches(12), Inches(0.35))
    write_block(
        box,
...
def title(slide, text, top=Inches(0.75), dark=False, width=Inches(11)):
    box = slide.shapes.add_textbox(Inches(0.7), top, width, Inches(1.3))
...
def lede(slide, text, top=Inches(2.0), dark=False):
    box = slide.shapes.add_textbox(Inches(0.7), top, Inches(11.5), Inches(0.9))
    write_block(box, [(text, 15, False, MUTED_ON_DARK if dark else INK_600, 0)])
...
def card(slide, left, top, width, height, heading, bullets, dark=False):
    ...
    box = slide.shapes.add_textbox(
        left + Inches(0.2), top + Inches(0.15), width - Inches(0.35), height - Inches(0.25)
    )
    tf = box.text_frame
    tf.word_wrap = True
    tf.clear()
    ...
    for b in bullets:
        ...
        r = p.add_run()
        r.text = "•  " + b
        set_run(r, 12, False, BODY_ON_DARK if dark else INK_700)
        p.space_after = Pt(4)
...
def stat_card(slide, left, top, width, height, value, label, dark=False):
    if dark:
        ...
        try:
            ...
            pass
    box = slide.shapes.add_textbox(
        left + Inches(0.2), top + Inches(0.2), width - Inches(0.35), height - Inches(0.3)
    )
    tf = box.text_frame
    tf.word_wrap = True
    tf.clear()
    ...
    r = p.add_run()
    r.text = label
    set_run(r, 12, False, MUTED_ON_DARK if dark else INK_600)


def callout(slide, text, title_text="Note", top=Inches(6.35), warn=False):
    add_rect(slide, Inches(0.7), top, Inches(0.08), Inches(0.7), AMBER if warn else GLASS)
    add_rect(
        slide,
        Inches(0.78),
        top,
    ...
    box = slide.shapes.add_textbox(Inches(1.0), top + Inches(0.08), Inches(11.3), Inches(0.55))
    tf = box.text_frame
    tf.word_wrap = True
    tf.clear()
    p = tf.paragraphs[0]
...
def build():
    ...
    add_round_rect(s, Inches(0.7), Inches(0.55), Inches(0.38), Inches(0.38), GLASS)
    name = s.shapes.add_textbox(Inches(1.2), Inches(0.55), Inches(3), Inches(0.4))
    ...
    box = s.shapes.add_textbox(Inches(0.7), Inches(6.0), Inches(11.9), Inches(0.9))
    write_block(
        box,
        [(
    ...
    card(s, Inches(0.7), Inches(3.1), Inches(3.85), Inches(2.9), "Reporting today", [
    ...
    card(s, Inches(4.75), Inches(3.1), Inches(3.85), Inches(2.9), "Transparency today", [
    ...
    card(s, Inches(8.8), Inches(3.1), Inches(3.85), Inches(2.9), "Service today", [
    ...
    lede(s, "Everything else — pass-through vs PEPM, shared savings, clinical modules — sits under these three.", top=Inches(2.05))
    pillars = [
        ("1", "Best-in-class reporting", "Sponsor dashboards, guarantee reconciliation, rebate waterfalls, trend drivers, design scenarios, and a data agent for ad-hoc questions — all computed from claims. Finance can drill to the dollar."),
        ("2", "Transparency with flexible pricing", "Claim-level derivation stays mandatory. How Glass gets paid is a menu: pass-through, PEPM, shared savings, outcomes at risk, or disclosed margin — always as named lines."),
        ("3", "Excellent service by chat and text", "Members get answers where they already are — chat and SMS — from an agent grounded in live eligibility, formulary, PA status, and claim history. Every figure comes from the book."),
    ]
    y = Inches(3.0)
    for n, h, body in pillars:
        add_round_rect(s, Inches(0.7), y, Inches(0.45), Inches(0.45), INK)
        num = s.shapes.add_textbox(Inches(0.7), y + Inches(0.05), Inches(0.45), Inches(0.4))
        write_block(num, [(n, 16, True, WHITE, 0)])
        for p in num.text_frame.paragraphs:
            p.alignment = PP_ALIGN.CENTER
        box = s.shapes.add_textbox(Inches(1.35), y - Inches(0.05), Inches(11.2), Inches(1.1))
        tf = box.text_frame
        tf.word_wrap = True
        tf.clear()
        p = tf.paragraphs[0]
    ...
    lede(s, "The questions a plan sponsor should answer at year-end — computed from the claim ledger, not supplied by the PBM as a finished answer.", top=Inches(2.05))
    table_slide(s, ["Report", "What it answers", "Why it is best-in-class"], [
    ...
    ], top=Inches(2.95), col_widths=[Inches(3.0), Inches(4.2), Inches(4.7)])
    callout(s, "No figure is a stored answer. If we cannot recompute it from claims and contract rules, it does not belong in a Glass report.", title_text="Reporting promise", top=Inches(6.45))

    # 5 Transparency
    s = blank_slide(prs)
    eyebrow(s, "Pillar two")
    title(s, "Transparency — without a single fee model", width=Inches(12))
    lede(s, "Transparency is the product layer. Pricing is packaging. Sponsors who need PEPM or shared savings still get claim-level derivation.", top=Inches(2.15))
    card(s, Inches(0.7), Inches(3.15), Inches(3.85), Inches(2.7), "Always on — Transparency layer", [
    ...
    card(s, Inches(4.75), Inches(3.15), Inches(3.85), Inches(2.7), "Your choice — Commercial shell", [
        "Full pass-through",
        "Transparent PEPM",
    ...
    card(s, Inches(8.8), Inches(3.15), Inches(3.85), Inches(2.7), "Rule — Disclosure", [
        "If Glass earns money, it appears as a named fee, share, credit, or disclosed margin",
        "Never an unexplained plan-vs-pharmacy gap",
    ])
    callout(s, "“Buy the fee model that fits your RFP. You still get the ledger. You just cannot buy opacity.”", title_text="Line for the room", top=Inches(6.2))

    # 6 Service
    s = blank_slide(prs)
    eyebrow(s, "Pillar three")
    title(s, "Excellent service through chat and text", width=Inches(12))
    lede(s, "Members should not need HR or a 45-minute hold to learn why a claim rejected. Glass answers in chat and SMS from the same rules engine that priced the claim.", top=Inches(2.15))
    ...
    table_slide(s, ["Channel", "Example", "Why it works"], [
    ...
    ], top=Inches(4.45), col_widths=[Inches(1.6), Inches(5.2), Inches(5.1)])

    # 7 Pricing
    s = blank_slide(prs)
    eyebrow(s, "Commercial menu")
    title(s, "Flexible pricing under the same glass", width=Inches(12))
    lede(s, "Reporting and service do not change with the fee model. Only how Glass appears on the invoice changes — always disclosed.", top=Inches(2.15))
    table_slide(s, ["Model", "How Glass is paid", "Reporting & service"], [
    ...
    ], top=Inches(3.1), col_widths=[Inches(3.2), Inches(4.4), Inches(4.3)])
    callout(s, "No unexplained plan-vs-pharmacy gap. Chat and text answers stay grounded in the book under every commercial model.", title_text="Non-negotiable", top=Inches(6.35))
    ...
    table_slide(s, ["Stop", "Show", "Pillar"], [
    ...
    ], top=Inches(2.95), col_widths=[Inches(1.2), Inches(8.2), Inches(2.5)])
    card(s, Inches(0.7), Inches(5.85), Inches(5.85), Inches(1.25), "For finance / benefits", [
        "Stay on reports and transparency until they trust the numbers — then show service.",
    ])
    card(s, Inches(6.75), Inches(5.85), Inches(5.85), Inches(1.25), "For HR / experience owners", [
    ...
    card(s, Inches(0.7), Inches(2.4), Inches(5.85), Inches(2.6), "Near-term engagement", [
    ...
    card(s, Inches(6.75), Inches(2.4), Inches(5.85), Inches(2.6), "What we need from you", [
    ...
    close = s.shapes.add_textbox(Inches(0.7), Inches(5.3), Inches(11.9), Inches(1.4))
    tf = close.text_frame
    tf.word_wrap = True
    tf.clear()
    p = tf.paragraphs[0]
